...
function CheckoutContainer({ children }: CheckoutProviderProps) {
   ...
   const activeStep = pathname.includes(paths.dashboard.customer_orders.checkout)
   ...
   const onChangeStep = useCallback(
      (type: 'back' | 'next' | 'go', step?: number) => {
         ...
         const redirectPath =
            targetStep === 0
               ? paths.dashboard.customer_orders.checkout
               : `${paths.dashboard.customer_orders.checkout}?${queryString}`;
